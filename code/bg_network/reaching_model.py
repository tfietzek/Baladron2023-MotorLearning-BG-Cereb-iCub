...
# input populations
S1 = ann.Population(geometry=parameters['dim_bg'], neuron=BaselineNeuron, name='S1')
...
M1_StrD1.connect_one_to_one(weights=0.2)

# connections Output
Brainstem_CPG = ann.Projection(pre=M1, post=Brainstem, target='exc', name='M1_Brainstem')
Brainstem_CPG.connect_one_to_one(weights=1.0)
...
StrD1_StrD1.connect_all_to_all(weights=0.05)

SNr_SNr = ann.Projection(pre=SNr, post=SNr, target='inh', synapse=ReversedSynapse, name='SNr_SNr')
